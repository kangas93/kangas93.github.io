...
function adaptToScreenSize() {
    ...
    var chart2SVG=document.getElementsByClassName("chart2");
    var timeLineOption=document.getElementById("selectDes");
   ...
   toolTip = d3.select('#toolTip')
    ...
    if(w > 1100 ){
        ...
        chart2[0].style.maxHeight = "90vh";
        stationCount[0].style.maxHeight = "95vh";
        chart2SVG[0].style.width="80vh";
        timeLineOption.style.left="110%";
        slider[0].style.right="10%";
        slider[0].style.top="10%";
...
function createStationBars(){
    ...
    .on('mouseover', function (d, i) {
        if(!subStations.includes(i.Name)){ 
            d3.select(this)
            .style("stroke-width",'0.5%').raise();
        }
        toolTip.transition()
             .duration(200)
             .style("opacity", 0.9);
        let num = i["Name"]+": "+(Math.round(i[year]).toString() + ' dagar');
        //let cc = alert_coords(d);
        
        //toolTip.html(num);
        //toolTip.style.left= (d.screenX + 10).toString() + "px";
        //toolTip.style.right= (d.screenY - 15).toString() + "px";
        //toolTip.style.opacity= "1";
        //document.getElementById("tooltip").style.left=(d.screenX + 10) + "px";
        //document.getElementById("tooltip").style.right=(d.screenY - 15) + "px";

        toolTip.html(num)
             .style("left", (d.pageX + 10) + "px")
             .style("top", (d.pageY - 15) + "px").raise();
   })
   .on('mouseout', function (d, i) {
    if(!subStations.includes(i.Name)){
        d3.select(this)
    ...
    barsFloor
        ...
        .on('mouseover', function (d, i) {
            ...
            toolTip.transition()
                 .duration(200)
                 .style("opacity", 0.9);
            var info = getStationInfo(i[year]);
            var dataCase = "Ingen data";
            if(info == dataCase){
                var num = i["Name"]+": "+info;
            }
            else{
